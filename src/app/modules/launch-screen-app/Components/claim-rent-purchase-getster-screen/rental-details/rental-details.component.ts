...
export class RentalDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._dataShare.download_test_user_id_data.subscribe((res) => {
      ...
      if (this.screen_id != undefined) {
        this.table.getrentaldetails(this.screen_id,this.country_code).subscribe({
          next: (response) => {
  ...
  @ViewChild('rentaldetails', { static: false })
  rentaldetails!: ElementRef;
  ...
  //         <span style="text-align: center;font-size:16px;color:black;font-weight:600;">Rental Details</span>
  ...
  public downloadAsPDF() {
    ...
    const htmlToPrint =
      ...
      '}' +
      '</style>';
    var printContents = document.getElementById('rentaldetails')!.innerHTML;
    let popupWin: any = window.open(
      'Angular Large Table to pdf',
      '_blank',
      'width=768,height=auto'
